...
DDL thực thi chính thức nằm trong `backend/migrations`. File này là bản mô tả hợp nhất của migrations `000001` đến `000004`; mọi thay đổi schema mới vẫn phải được tạo thành migration mới.
...
erDiagram
    ...
    APP_USERS ||--o{ ASSET_CHANGES : replicates
...
CREATE TABLE asset_changes (
  ...
  change_type TEXT NOT NULL CHECK (change_type IN ('upsert', 'trash', 'restore', 'delete')),
  asset_snapshot JSONB,
  changed_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CHECK (
    (change_type = 'delete' AND asset_snapshot IS NULL) OR
    (change_type <> 'delete' AND asset_snapshot IS NOT NULL)
  )
);
```
...
- `asset_changes.asset_id` không có foreign key tới `assets`, nhờ đó delete tombstone vẫn tồn tại sau hard delete.
- Change khác `delete` luôn có metadata snapshot; change `delete` luôn có snapshot null.
